...
here::i_am("pedro_pipeline_code-NVP-intercal-DL.R")
...
setwd(here("/Users/nastassiapatin/OneDrive - SCCWRP/Experiments/Intercalibration/dloop"))

#calling function for LCA
source("/Users/Home/OneDrive - SCCWRP/Experiments/Intercalibration/dloop/MURI-BLAST-nt_euk/code_etc/LCA_function_pedro_1.3-NVP.R")

#the folder in which a subfolder, called `Fastq` lives, and which contains only fastq or fastq.gz files
PARENT_LOCATION <- "/Volumes/CalCOFI_OMICS/Sequence_data/UCSD_NovaSeq_20240403/intercal-dloop/"
...
RUN_NAME <- "Intercal-dloop" #basename(PARENT_LOCATION) #
...
PROCESSED_LOCATION <- "/Users/nastassiapatin/OneDrive - SCCWRP/Experiments/Intercalibration/dloop/MURI-BLAST-nt_euk/"
...
#check dependencies and get path for taxonkit
  TAXONKIT_PATH = "/Users/Home/miniconda3/bin/taxonkit" # office
...
#read in primer data
primer.data <- read.csv("/Users/Home/OneDrive - SCCWRP/Bioinformatics/MURI metabarcoding/Accessory_scripts/primer.data.csv")
...
###COPY FILES TO SCRIPTS FOLDER and write primer-trimming script via make_primer_shell_script.R
    ...
    #system2("cp", args = c(shQuote(here("Accessory_scripts", 
     #                                   "make_primer_shell_script.R")), 
      #                     shQuote(CODE_LOCATION))) 
    ...
    #system2("cp", args = c(shQuote(here("code/draft_pipeline_code.R")), shQuote(CODE_LOCATION))) #this file as in rpk laptop
    #system2("cp", args = c(shQuote(here("Accessory_scripts", 
     #                                   "pedro_pipeline_code-NVP.R")), 
      #                     shQuote(CODE_LOCATION))) # NVP's version
  
    if (BLASTSCOPE == "vertebrate"){
      system2("cp", args = c(shQuote("/Users/Home/OneDrive - SCCWRP/Bioinformatics/MURI metabarcoding/Accessory_scripts/MURIblast_vertebrate_template-NVP-localnteuk.sh"), 
...
sample.names <- sapply(strsplit(basename(fnFs), "_DL"), `[`, 1)
...
filterAndTrim(fnFs[[51]], filtFs[[51]], fnRs[[51]], filtRs[[51]], 
                     trimRight = c(PRIMERLENGTH_R,PRIMERLENGTH_F),
                     maxN=0, maxEE=c(2,2), truncQ=2, rm.phix=TRUE,
...
#dadaFs.lrn <- dada(filtFs, err=NULL, selfConsist = TRUE, 
#                   multithread=TRUE, MAX_CONSIST=20)
#errF <- dadaFs.lrn[[1]]$err_out
#dadaRs.lrn <- dada(filtRs, err=NULL, selfConsist = TRUE, 
 #                  multithread=TRUE, MAX_CONSIST=20)
...
fqF1 <- readFastq(filtFs[[19]])
fqF2 <- readFastq(filtFs[[20]])
fqR1 <- readFastq(filtRs[[19]])
fqR2 <- readFastq(filtRs[[20]])

sapply(c(fqF1, fqF2, fqR1, fqR2), length)
### Merge Paired Reads ---------------------------------------------------------------------
...
head(track)
write.csv(track, paste0(PROCESSED_LOCATION, "tracking_reads.csv"))
...
Hashes <- map_chr (colnames(seqtab.nochim.df), 
                   ~ digest(.x, algo = "sha1", serialize = T, 
                            skip = "auto")) # original code had serialize=F ??
...
system2("cp", args = c("-r", shQuote(paste0(PARENT_LOCATION, "code_etc")), 
                       shQuote(paste0(PROCESSED_LOCATION, "/", PRIMERNAME, "_", RUN_NAME))))
system2("cp", args = c("-r", shQuote(paste0(PARENT_LOCATION, "outputs")), 
                       shQuote(paste0(PROCESSED_LOCATION, "/", PRIMERNAME, "_", RUN_NAME))))
system2("cp", args = c("-r", shQuote(paste0(PARENT_LOCATION, "logs")), 
                       shQuote(paste0(PROCESSED_LOCATION, "/", PRIMERNAME, "_", RUN_NAME))))
...
system2("sh", args = shQuote(paste0(CODE_LOCATION, 
                                    "/MURIblast_vertebrate_template-NVP-localnteuk-office.sh")))
...
if (file.size(paste0(OUTPUT_LOCATION, "/", "BLAST_output.txt")) == 0L){
  ...
  db <- LCA(BLASTOUTPUT = paste0(PROCESSED_LOCATION, "/", PRIMERNAME, "_", RUN_NAME, "/outputs/new_annotations.txt"),
            FASTA = paste0(PROCESSED_LOCATION, "/", PRIMERNAME, "_", RUN_NAME, "/outputs/seqs_to_annotate.fasta"))
